...
void KDTree::buildTreeBF(const uint64_t subarray_begin, const uint64_t subarray_end, unsigned int depth) {
    uint64_t range = subarray_end - subarray_begin;
    ...
    this->buildTree(
        subarray_begin + 1,
        subarray_begin + median + 1,
        (depth + 1) % this->num_dimensions
    );
...
KNNQueue KDTree::nearestNeighborsSearchBF(const float* query_point, const uint64_t& num_neighbors) const {
    KNNQueue queue(query_point, num_neighbors, this->num_dimensions);

    this->nearestNeighborsSearchBF(query_point, 0, this->num_points, 0, queue);
...
void KDTree::nearestNeighborsSearchBF(const float* query_point, uint64_t begin, uint64_t end, uint64_t depth, KNNQueue& nearest_neighbors) const {
    uint64_t range = end - begin;
    uint64_t traverser_index = begin;
...
//    }
    ...
    if (query_at_current_dimension < traverser_at_current_dimension) {
        ...
        this->nearestNeighborsSearch(query_point, begin + (range / 2) + 1, end, (depth + 1) % this->num_dimensions, nearest_neighbors);
    }
